...
```
. <(wget -qO- https://raw.githubusercontent.com/L0vd/Nolus/main/Monitoring/nolus-monitoring-install.sh)
```
...
# make the telegraf user sudo and adm to be able to execute scripts as nolus user
...
```
cd $HOME
mkdir nolus-monitoring
cd nolus-monitoring
wget https://raw.githubusercontent.com/L0vd/Nolus/main/Monitoring/monitor.sh
wget https://raw.githubusercontent.com/L0vd/Nolus/main/Monitoring/telegraf.conf
wget https://raw.githubusercontent.com/L0vd/Nolus/main/Monitoring/variables.sh
```
...
* full path to nolus binary to COS_BIN_NAME ( check ```which nolusd```)
* node PRC port to COS_PORT_RPC ( check in file ```path_to_nolus_node_config/config/config.toml```)
* node validator address to COS_VALOPER ( like ```nolusvaloper********```)
...
Create telegraf service for nolus-node monitoring
```
cat > /etc/systemd/system/telegraf_nolus.service <<EOL
[Unit]
...
User=telegraf
ExecStart=/usr/bin/telegraf -config /etc/telegraf/telegraf_nolus.conf -config-directory /etc/telegraf/telegraf.d $TELEGRAF_OPTS
...
sudo mv telegraf.conf /etc/telegraf/telegraf_nolus.conf
```
Restart telegraf service

```
sudo systemctl daemon-reload
sudo systemctl enable telegraf_nolus
sudo systemctl restart telegraf_nolus
```
...
## [Dashboard link](http://95.216.2.219:3000/d/Nolus/nolus-monitoring-by-l0vd?orgId=1&refresh=30s&from=now-5m&to=now)
