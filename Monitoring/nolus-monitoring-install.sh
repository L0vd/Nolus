...
echo ''
echo -e 'INSTALLING NOLUS NODE MONITORING'
...
cd nolus-monitoring
wget https://raw.githubusercontent.com/L0vd/Nolus/main/Monitoring/monitor.sh >/dev/null 2>&1
...
#Nolus monitoring variables template 
COS_BIN_NAME=$COS_BIN_NAME             # example: /root/go/bin/nolusd or /home/user/go/bin/nolusd
COS_PORT_RPC=$COS_PORT_RPC         # default: 26657
COS_VALOPER=$COS_VALOPER           # example: nolus@valoper1234545636767376535673
LOGENTRY=$LOGENTRY
EOL
...
[[outputs.influxdb]]
  database = "nolusmetricsdb"
  urls = [ "http://95.216.2.219:8086" ] 
  username = "metric" 
  password = "password" 
[[inputs.exec]]
  commands = ["sudo su -c /root/nolus-monitoring/monitor.sh -s /bin/bash root"] # change path to your monitor.sh file and username to the one that validator runs at (e.g. root)
  interval = "15s"
  timeout = "5s"
  data_format = "influx"
  data_type = "integer"
...
sudo mv telegraf_nolus.conf /etc/telegraf/telegraf_nolus.conf
...
echo ''
if [[ `sudo systemctl status telegraf_nolus | grep active` =~ "running" ]]; then
...
echo ''
echo -e '\e[7mYour Nolus node monitoring is installed!\e[0m'
echo ''
...
echo -e ''
echo -e 'Check telegraf logs: \e[7msudo journalctl -u telegraf_nolus -f\e[0m'
